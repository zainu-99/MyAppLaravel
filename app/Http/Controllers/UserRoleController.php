...
class UserRoleController extends Controller
{
    ...
    public function index(Request $request,$iduser)
    {
        ...
        $list =Role::selectRaw("roles.*,c.isview,c.isadd,c.isedit,c.isdelete,c.isprint,c.iscustom,b.allow_view,b.allow_add,b.allow_edit,b.allow_delete,b.allow_print,b.allow_custom")->leftJoin(DB::raw("(select * from user_role where id_user='".$id_user."') as b"),"b.id_role","roles.id")->leftJoin('role_group_level as c','roles.id','c.id_role')->OrderBy("roles.url")->OrderBy("roles.name")->where('c.id_group_level',$id_group_level)->whereRaw('1 in (isview,isadd,isedit,isdelete,isprint,iscustom)')->get();
